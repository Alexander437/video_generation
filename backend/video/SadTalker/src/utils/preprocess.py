...
class CropAndExtract():
    ...
    def generate(self, input, save_dir, crop_or_resize='crop', source_image_flag=False, pic_size=256):
        if isinstance(input, str):
            pic_name = os.path.splitext(os.path.split(input)[-1])[0]

            # load input
            if not os.path.isfile(input):
                raise ValueError('input_path must be a valid path to video/image file')
            elif input.split('.')[-1] in ['jpg', 'png', 'jpeg']:
                # loader for first frame
                full_frames = [cv2.imread(input)]
                fps = 25
            else:
                # loader for videos
            ...
            pic_name = strftime("%Y_%m_%d_%H.%M.%S")
            full_frames = [input]
            fps = 25

        else:
            raise ValueError('input must be a file or numpy array')
