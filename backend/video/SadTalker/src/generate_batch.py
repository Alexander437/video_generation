...
def get_data(first_coeff_path, audio_data, device, ref_eyeblink_coeff_path, still=False, idlemode=False, length_of_audio=False, use_blink=True):
    ...
    if idlemode:
        ...
        if not isinstance(audio_data, np.ndarray):
            wav = audio.load_wav(audio_data, 16000)
            audio_name = os.path.splitext(os.path.split(audio_data)[-1])[0]
        else:
            wav = audio_data
            audio_name = strftime("%Y_%m_%d_%H.%M.%S")
